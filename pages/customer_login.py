...
class CustomerLogin(BasePage):
    ...
    def fill_login_form(self, login, password):
        ...
        password_field = self.find(loc.password_field_doc)
    ...
    def check_error_alert_text_is(self, text):
        # error_locator = self.find(loc.error_locator)
        WebDriverWait(self.driver, 10).until(project_ec.text_is_not_empty_in_element(loc.error_locator))
